...
var category = {OBJ: $("#_JD_ALLSORT"),
		//URL_Serv: "http://manage.letus.com/web/itemcat/all?callback=category.getDataService",
        ...
        return "" == c ? d : (/^http[s]?:\/\/([\w-]+\.)+[\w-]+([\w-.\/?%&=]*)?$/.test(c) || (c = c.replace(/-000$/, ""), c = c.match(/^\d*-\d*$/) ? "http://channel.jd.com/" + c + ".html" : "http://www.letus.com" + c + ".html"), '<a href="' + c + '">' + d + "</a>")
       ...
       /* var a, b = {}, c = $("#categorys-2013");
    	...
    	//使用jsonp来实现跨域请求 category.getDataService回调函数
